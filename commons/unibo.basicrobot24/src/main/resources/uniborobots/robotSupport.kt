...
object robotSupport{
	...
	fun create( owner: ActorBasic, configFileName: String, endPipe: ActorBasic? = null ){
		...
		CommUtils.outred("robotSupport | ++++++++++++++++++++ create vrenv=$vrenv configFileName=$configFileName" )
...
//			"realmbot"   -> { return false 	}	//TODO
		...
		//MODIFICA savignano
	fun dostepback( time : Long ) : Boolean {
		when( robotKind ){
			"virtual"    -> { return vr.stepback(  time  ) }
			"realnano"   -> {
				robotNano.nanoSupport.move( "s" ) // Usa il comando 's' per il robot reale
				Thread.sleep(time)
				robotNano.nanoSupport.move( "h" )
				return true
			}
			else         -> {
				println("		--- robotSupport: dostepback | robot unknown")
				return false
			}
		}
	}
