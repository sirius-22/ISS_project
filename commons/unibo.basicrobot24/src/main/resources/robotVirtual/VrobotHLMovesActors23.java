...
public class VrobotHLMovesActors23 extends ApplAbstractObserver implements IVrobotMovesAsynch {
    ...
    public VrobotHLMovesActors23(String vitualRobotIp, ActorBasic owner) {
        ...
        this.wsCommSupport = WsConnection.create(vitualRobotIp+":8091","api/moves");
        this.owner          = owner;
    ...
    public void move( String cmd ) throws Exception{
        ...
        else if( cmd.equals("z") ) forward( 100 );
    }
    ...
    public void backward(int time) throws Exception {
        // la chiamata asincrona probabilmente da errore perchè non ne riesce a gestire tante
        //startTimer();
        //wsCommSupport.forward(VrobotMsgs.backwardcmd.replace("TIME", "" + time));

        //chiamata sincrona
        sendSynchToWenv(VrobotMsgs.backwardcmd.replace("TIME", "" + time));
    }
...
// Observer part
    ...
    public void update(String info) {
         try {
            ...
            */
                CommUtils.outred("     VrobotHLMovesActors23 | update Collision!!! info : " + info);
                return;
            }
    ...
    public boolean stepback(long time) throws Exception {
        doingStep = true;
        // Usiamo lo stesso meccanismo sincrono di step(), ma con il comando backward
        String cmd    = VrobotMsgs.backwardcmd.replace("TIME", "" + time);
        String result = sendSynchToWenv(cmd);
        doingStep = false;
        return result.contains("true");
}
